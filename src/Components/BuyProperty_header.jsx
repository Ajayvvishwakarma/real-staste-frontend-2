...
const API_URL = "http://localhost:8000/api/available-cities";

const BuyPropertyHeaderIntegration = () => {
  const { city, locality: urlLocality } = useParams();
  const [availableCities, setAvailableCities] = useState([]);
  const [selectedType, setSelectedType] = useState("Residential plot");
  const [showTypeDropdown, setShowTypeDropdown] = useState(false);
    ...
    >
      ...
      <div className="flex ml-38 flex-col md:flex-row md:space-x-4">
        ...
        <div className="relative">
          ...
          {showTypeDropdown && (
            <div className="absolute left-0 top-full mt-2 w-[350px] bg-white rounded-xl shadow-2xl z-30 p-6">
              <div className="font-sm text-gray-800 mb-2 text-sm">
                RESIDENTIAL
              </div>
              <div className="flex items-center gap-2 mb-4">
                <span className="bg-[#e3f3ff] px-4 py-2 rounded-full text-black font-sm flex items-center">
        ...
        <div className="relative">
          ...
          {showBudgetDropdown && (
            <div className="absolute left-0 top-full mt-2 w-[354px] bg-white rounded-xl shadow-2xl z-30">
              <div className="pt-4 pb-2 px-4">
                <div className="flex justify-between mb-1">
                  <span className="font-semibold text-gray-800">Min</span>
                  <span className="font-semibold text-gray-800">Max</span>
                ...
                <div className="flex gap-8">
                  <div className="flex flex-col gap-1 w-1/2 pr-2">
                    {["4000", "6000", "10,000", "15,000", "20,000"].map(
                      (val) => (
                        <label
                          key={val}
                        ...
                        >
                          <input
                            ...
                            name="minBudget"
                            value={val}
                            checked={minBudget === val}
                            onChange={() => setMinBudget(val)}
                          />
                          <span>{val}</span>
                        </label>
                      )
                  ...
                  <div className="flex flex-col gap-1 w-1/2 pl-2">
                    {["40,000", "60,000", "80,000", "1 lac", "2 lacs"].map(
                      (val) => (
                        <label
                          key={val}
                          className="flex items-center gap-2 cursor-pointer text-gray-700 text-sm"
                        >
                          <input
                            ...
                            name="maxBudget"
                            value={val}
                            checked={maxBudget === val}
                            onChange={() => setMaxBudget(val)}
                          />
                          <span>{val}</span>
                        </label>
                      )
        ...
        <div className="relative">
          ...
          {showOwnershipDropdown && (
            <div className="absolute left-0 top-full mt-2 w-[358px] bg-white rounded-xl shadow-2xl z-30">
              <div className="pt-6 pb-2 px-4">
                <div className="flex justify-between items-center mb-2">
                  {["Individual", "Agent", "Builder"].map((val) => (
                    <button
                      ...
                      className={`px-6 py-2 rounded-full border text-sm font-medium transition-colors ${
                        selectedOwnership === val
                          ? "bg-[#e3f3ff] border-blue-400 text-blue-700"
...
export default BuyPropertyHeaderIntegration;