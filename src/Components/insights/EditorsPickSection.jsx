...
const editorsPicks = [
  {
    ...
    category: "Investment Guide",
    excerpt:
      "Discover the hidden gems in Mumbai's real estate market that offer excellent investment potential with upcoming infrastructure development.",
    publishDate: "Dec 15, 2024",
    featured: true,
  },
  {
    ...
    category: "Location Guide",
    excerpt:
      "Complete guide to choosing the perfect location in Bangalore's IT corridor for working professionals looking for convenience and connectivity.",
    publishDate: "Dec 12, 2024",
    featured: false,
  },
  {
    ...
    category: "Market Analysis",
    excerpt:
      "How the expanding Delhi Metro network is reshaping property values across different sectors in Gurgaon, Noida, and Faridabad.",
    publishDate: "Dec 10, 2024",
    featured: true,
  },
  {
    ...
    category: "Investment Guide",
    excerpt:
      "Analysis of upcoming IT parks in Pune and their impact on surrounding residential property markets for smart investors.",
    publishDate: "Dec 8, 2024",
    featured: false,
  },
];

const EditorsPickSectionIntegration = () => {
  const sliderRef = useRef(null);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Curated insights from our experts
          </p>
        </div>
        ...
        <div className="relative">
          ...
          >
            {editorsPicks.map((pick) => (
              <div
                key={pick.id}
                className={`bg-gradient-to-br from-blue-50 to-white rounded-xl shadow-lg hover:shadow-2xl transition-shadow cursor-pointer flex flex-col justify-between ${
                  pick.featured
                    ? "xl:max-w-[400px] xl:min-w-[350px]"
                    : "max-w-[260px]"
                }`}
                style={{
                  minWidth: pick.featured ? "350px" : "200px",
                  scrollSnapAlign: "start",
                }}
              >
                {/* Image Section */}
                <div className="relative w-full aspect-[16/9] overflow-hidden">
                  ...
                  />
                  <div className="absolute top-3 left-3 bg-blue-600 text-white px-3 py-1 rounded-full shadow text-xs font-semibold">
                    {pick.category}
                  </div>
                  {pick.featured && (
                    <div className="absolute top-3 right-3 bg-yellow-500 text-white px-3 py-1 rounded-full shadow text-xs font-semibold">
                      Featured
                    </div>
                  )}
                </div>
                ...
                {/* Content Section */}
                <div className="p-5">
                  <div className="flex items-center text-xs text-blue-500 gap-x-2 font-semibold mb-2">
                    <span>{pick.publishDate}</span>
                    <span>•</span>
                    <span>{pick.readTime}</span>
                  </div>
...
export default EditorsPickSectionIntegration;