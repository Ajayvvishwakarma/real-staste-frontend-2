...
import React, { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import InsightsNavIntegration from './Insights_nav';
import UpperFooter from '../UpperFooter';
import LowerFooter from '../LowerFooter';
import {
  ChartBarIcon,
...
const InsightsLayoutIntegration = ({ children }) => {
  const location = useLocation();
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);
  const [showMobileSidebar, setShowMobileSidebar] = useState(false);
  ...
  const sidebarLinks = [
    { title: 'Insights', icon: <HomeIcon className="w-5 h-5" />, path: '/insights' },
    { title: 'Price Trends', icon: <ChartBarIcon className="w-5 h-5" />, path: '/insights/price-trends' },
    { title: 'My Property Insights', icon: <HomeIcon className="w-5 h-5" />, path: '/insights/my-property' },
    { title: 'Articles', icon: <NewspaperIcon className="w-5 h-5" />, path: '/insights/articles' },
    { title: 'News', icon: <NewspaperIcon className="w-5 h-5" />, path: '/insights/news' },
    { title: 'Guides', icon: <BookOpenIcon className="w-5 h-5" />, path: '/insights/guides' },
    { title: 'Budget Calculator', icon: <CalculatorIcon className="w-5 h-5" />, path: '/insights/budget-calculator' },
    { title: 'EMI Calculator', icon: <HomeIcon className="w-5 h-5" />, path: '/insights/emi-calculator' },
    { title: 'Home Loan Eligibility Calculator', icon: <HomeIcon className="w-5 h-5" />, path: '/insights/home-loan-eligibility' },
    { title: 'Area Convertor Tool', icon: <ArrowsRightLeftIcon className="w-5 h-5" />, path: '/insights/area-converter' },
  ];

  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100">
      <InsightsNavIntegration />
      <div className="flex pt-[72px] min-h-screen">
                ...
                return (
                  <Link
                    ...
                    style={{
                      ...
                      whiteSpace: 'nowrap',
                      maxWidth: isCollapsed ? '3.5rem' : '12rem'
                    }}
                  >
                    <div className="flex items-center space-x-3 relative w-full">
                      {React.cloneElement(link.icon, {
...
export default InsightsLayoutIntegration;