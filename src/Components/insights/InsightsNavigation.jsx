...
const InsightsNavigationIntegration = () => {
  const navigate = useNavigate();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {navigationItems.map((item, index) => (
          <div
            key={item.title}
            onClick={() => navigate(item.path)}
            className="flex items-center p-4 bg-gray-50 hover:bg-blue-100 rounded-lg cursor-pointer transition-colors duration-200 group"
          >
            <span className="text-3xl mr-3 transition-transform duration-200 group-hover:scale-125">{item.icon}</span>
            <div>
              <h3 className="font-medium text-gray-800">{item.title}</h3>
            </div>
          </div>
...
export default InsightsNavigationIntegration;