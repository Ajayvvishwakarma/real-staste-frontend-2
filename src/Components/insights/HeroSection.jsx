...
const HeroSectionIntegration = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // You can add navigation or API logic here
    if (searchQuery.trim()) {
  ...
  return (
    <>
      ...
      {/* Search Section */}
      <form
        className="max-w-sm xs:max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl mx-auto mb-4 xs:mb-5 sm:mb-6"
      ...
      >
        <div className="flex flex-col sm:flex-row gap-1 xs:gap-2 bg-white rounded-lg shadow-lg p-2 xs:p-3">
          ...
          <button
            type="submit"
            className="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-3 xs:px-4 py-1.5 xs:py-2 rounded-lg transition-colors duration-200 text-xs xs:text-sm min-h-[36px] xs:min-h-[40px]"
          >
            Search Insights
          </button>
        </div>
