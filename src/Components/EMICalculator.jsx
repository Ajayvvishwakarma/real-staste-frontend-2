...
// Replace with your backend API endpoint for EMI calculations (if applicable)
const API_URL = "http://localhost:8000/api/emi-calculator";

const EMICalculatorIntegration = ({ isOpen, onClose }) => {
  const [formData, setFormData] = useState({
    loanAmount: 3000000, // ₹30,00,000
    tenure: 20, // years
    interestRate: 8.9, // %
  });
  ...
  const calculateEMIIntegration = async () => {
    setLoading(true);
    setError("");
    try {
  ...
  // Fallback calculation if backend is not used
  useMemo(() => {
    ...
    const totalPayable = emi * formData.tenure * 12;
    const interestAmount = totalPayable - formData.loanAmount;

    setResults({ emi, totalPayable, interestAmount });
  }, [formData]);

  const pieData = [
    { name: "Principal Amount", value: formData.loanAmount, color: "#10B981" },
    { name: "Interest Amount", value: results.interestAmount, color: "#059669" },
  ];
      ...
      <div className="bg-white w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-lg shadow-2xl relative">
        ...
        <div className="p-6 md:p-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col justify-center">
              {loading ? (
                <p className="text-center text-green-600">Calculating...</p>
              ) : error ? (
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-6 mb-6 border border-green-200">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center">
                        <p className="text-gray-600 text-sm">Total Interest</p>
                        <h3 className="text-xl font-semibold text-gray-800">
                          ₹ {Math.round(results.interestAmount).toLocaleString("en-IN")}
                        </h3>
                      </div>
                    </div>
                  </div>
