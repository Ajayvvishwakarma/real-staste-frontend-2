...
const MARKETING_API_URL = "http://localhost:8000/api/marketing";

import Admin_Navbar from './Admin_Navbar';
import Admin_Sidebar from './Admin_Sidebar';

const MarketingManagementIntegration = () => {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [isLargeScreen, setIsLargeScreen] = useState(window.innerWidth >= 1024);
  const [selectedTab, setSelectedTab] = useState('campaigns');
  const [campaigns, setCampaigns] = useState([]);
  ...
  useEffect(() => {
    const fetchMarketingData = async () => {
      setLoading(true);
      try {
        const campaignsResponse = await fetch(`${MARKETING_API_URL}/campaigns`);
        const templatesResponse = await fetch(`${MARKETING_API_URL}/email-templates`);
        const seoResponse = await fetch(`${MARKETING_API_URL}/seo-settings`);

        if (!campaignsResponse.ok || !templatesResponse.ok || !seoResponse.ok) {
            ...
            <div className="mb-8">
              <div className="border-b border-gray-200">
                <nav className="-mb-px flex space-x-4 lg:space-x-8 overflow-x-auto">
                  ...
                  <button onClick={() => setSelectedTab('seo')}>SEO Settings</button>
                </nav>
              </div>
            </div>
...
export default MarketingManagementIntegration;