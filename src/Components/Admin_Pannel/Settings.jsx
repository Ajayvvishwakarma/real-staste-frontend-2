...
const SETTINGS_API_URL = "http://localhost:8000/api/settings";
...
const SettingsIntegration = () => {
  ...
  const handleSaveSettings = async (tabId) => {
    try {
      setLoading(true);
      const response = await fetch(`${SETTINGS_API_URL}/${tabId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(settings[tabId]),
      });
      if (!response.ok) {
        throw new Error(`Failed to save settings for ${tabId}`);
      }
      const savedData = await response.json();
      setSettings(prev => ({ ...prev, [tabId]: savedData }));
      setSuccessMessage(`${tabId} settings saved successfully!`);
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (error) {
      console.error('Error saving settings:', error);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-16 min-h-screen transition-all duration-300" style={{ paddingLeft: isLargeScreen ? '16rem' : '0' }}>
        <main className="bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
            ...
            <div className="mt-8 bg-white rounded-lg shadow-sm border border-gray-200">
              <div className="border-b border-gray-200">
                <nav className="flex space-x-4 px-4">
                  <button onClick={() => setActiveTab('general')} className="py-4 text-sm">General</button>
                  <button onClick={() => setActiveTab('email')} className="py-4 text-sm">Email</button>
                  <button onClick={() => setActiveTab('payment')} className="py-4 text-sm">Payment</button>
                </nav>
              </div>
