...
// Optionally fetch menu/badge counts from backend
// const MENU_API_URL = "http://localhost:8000/api/admin-sidebar-menu";

const AdminSidebarIntegration = ({ sidebarOpen, setSidebarOpen }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [expandedMenus, setExpandedMenus] = useState({});
  const [isCollapsed, setIsCollapsed] = useState(false);

  // Optionally load dynamic menu items from backend
  // For now, use static definition (can be replaced with state/useEffect fetching from API)
  const menuItems = [
    {
      title: 'Dashboard',
      path: '/admin/dashboard',
...
export default AdminSidebarIntegration;