...
const ANALYTICS_API_URL = "http://localhost:8000/api/analytics"; // Replace with your backend API
...
const AnalyticsIntegration = () => {
  ...
  useEffect(() => {
    const path = location.pathname;
    if (path.includes('/users')) setActiveView('users');
    else if (path.includes('/properties')) setActiveView('properties');
    else if (path.includes('/financial')) setActiveView('financial');
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${ANALYTICS_API_URL}?range=${selectedDateRange}`);
        if (!response.ok) {
        ...
        setAnalyticsData(null); // Fallback to null if data fetching fails
      } finally {
        setLoading(false);
      }
    };
  ...
  const ChartComponent = ({ type }) => {
    if (!analyticsData) return <div className="text-center text-gray-500">Data not available</div>;
