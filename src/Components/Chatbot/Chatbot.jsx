...
const ChatbotIntegration = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Hello! Welcome to our platform! 👋 How can I assist you today?",
      sender: 'bot',
      timestamp: new Date().toLocaleTimeString()
    }
  ]);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const botMessage = {
        id: Date.now() + 1,
        text: getBotResponse(inputMessage.toLowerCase()),
        sender: 'bot',
        timestamp: new Date().toLocaleTimeString()
      };
      setMessages(prev => [...prev, botMessage]);
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-50">
        ...
        {isOpen && (
          <div className="bg-white rounded-lg shadow-lg w-80 sm:w-96 h-[500px] flex flex-col border border-gray-200">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
              {messages.map(message => (
                <div
                  ...
                  className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}
                >
                  <div className={`${message.sender === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'} p-3 rounded-lg max-w-xs`}>
                    <p>{message.text}</p>
