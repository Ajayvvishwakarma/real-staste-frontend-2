...
const SubmitYourPropertyIntegration = () => {
  const [step, setStep] = useState(1);
  const [form, setForm] = useState({
    propertyFor: "Buy",
    propertyType: "",
  ...
  const handleNext = (e) => {
    e.preventDefault();
    if (step === 1 && (!form.propertyType || !form.city)) {
      alert("Please fill all required fields.");
      return;
    }
    if (step === 2 && !form.mobile) {
      alert("Please enter your mobile number.");
      return;
    }
    setStep((prev) => Math.min(prev + 1, 3));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await submitPropertyInquiry(form);
      if (result.success) {
        alert(`Thank you! Your inquiry has been submitted successfully. Requirement ID: ${result.id}`);
        setStep(1);
        setForm({
          propertyFor: "Buy",
          propertyType: "",
  ...
  return (
    <div className="w-full min-h-screen bg-[#f7f7f7] flex flex-col items-center py-10 px-4">
      <div className="max-w-7xl w-full mx-auto">
        <h2 className="text-center text-xl md:text-2xl font-bold text-[#f75c5c] mb-2">
          Share Your Property Requirements!
        </h2>
        <p className="text-center text-gray-700 mb-6">
          We’ll find the best matching properties for you.
        </p>
        {/* Stepper */}
        <div className="flex justify-center items-center mb-8">
          <div className="relative w-full max-w-4xl">
            ...
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center justify-center relative z-10">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold border-2 ${
                    step === s ? "bg-[#1756a9] text-white border-[#1756a9]" : "bg-white text-[#1756a9] border-gray-400"
                  }`}
                >
                  {s}
                </div>
            ...
            <div
              className="absolute top-6 left-0 h-[2px] bg-[#1756a9]"
        ...
        {/* Step 1: Details */}
        {step === 1 && (
          <form onSubmit={handleNext} className="bg-white shadow-md rounded-md p-6 mb-6">
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700">Property For</label>
              <div className="flex gap-4 mt-2">
                {["Buy", "Rent", "PG"].map((option) => (
                  <label key={option} className="flex items-center gap-2">
                    <input
                      type="radio"
                      name="propertyFor"
                      value={option}
                      checked={form.propertyFor === option}
                      onChange={handleStepChange}
                      className="accent-[#1756a9]"
                    />
                    {option}
                  </label>
                ))}
            ...
            <button
              type="submit"
              className="w-full mt-4 bg-[#1756a9] text-white py-2 rounded-md hover:bg-[#2ca3e6] transition"
            >
              Next
            </button>
          </form>
        ...
        {step === 2 && (
          <form onSubmit={handleNext} className="bg-white shadow-md rounded-md p-6 mb-6">
            ...
            <button
              type="button"
              onClick={handlePrev}
              className="w-full mt-2 bg-gray-200 text-[#1756a9] py-2 rounded-md"
            >
              Back
            </button>
          </form>
          ...
          <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-md p-6 mb-6">
            ...
            <div className="mb-4 flex items-center">
              <input
                ...
                checked={form.agree}
                onChange={handleStepChange}
                className="rounded border-gray-300 accent-[#1756a9]"
              />
...
export default SubmitYourPropertyIntegration;