...
const PricingAndBenefitsIntegration = () => {
  const PACKAGES = [
    {
      id: "advanced",
      title: "Advanced",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-200 via-white to-green-200">
      {/* Header Section */}
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto py-6 sm:py-8">
        <header className="text-center">
          <h2 className="font-extrabold text-xl sm:text-2xl md:text-3xl lg:text-4xl text-black leading-tight">
            ...
            <br />
            <span className="block text-green-500 mt-1 sm:mt-2">
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto py-6 sm:py-8">
        <div className="flex gap-4 overflow-x-auto pb-4">
          {PACKAGES.map((pkg, idx) => (
            <div key={idx} className="min-w-[280px] bg-white p-4 rounded-md shadow-md">
              ...
              <div className="mt-4">
                {pkg.prices.map((price, idx) => (
                  <div
                    key={idx}
                    className={`p-2 border ${selected[pkg.id] === idx ? "border-green-600" : "border-gray-200"}`}
                    onClick={() => setSelected({ ...selected, [pkg.id]: idx })}
                  >
                    <p className="font-bold">{price.months}</p>
                    <p className="text-sm text-gray-600">Total: {rupee(price.total)}</p>
                    <p className="text-sm text-gray-600">Per Month: {rupee(price.perMonth)}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4">
        ...
        {faqs.map((faq, idx) => (
          <div key={idx} className="border-b py-2">
            <button onClick={() => setOpenIndex(openIndex === idx ? null : idx)} className="w-full text-left font-semibold">
              {faq.q}
            </button>
            {openIndex === idx && <p className="text-gray-600 mt-2">{faq.a}</p>}
          </div>
        ))}
      </section>
    </div>
...
export default PricingAndBenefitsIntegration;