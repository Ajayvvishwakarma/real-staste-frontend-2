...
const AuthModalIntegration = ({ isOpen, onClose, initialMode = 'login' }) => {
  const [mode, setMode] = useState(initialMode); // 'login' or 'signup'
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [formData, setFormData] = useState({
        ...
        <div className="relative bg-white sm:rounded-2xl shadow-xl w-full sm:max-w-md mx-auto transform transition-all h-full sm:h-auto sm:max-h-[95vh] overflow-y-auto">
          ...
          <div className="px-6 py-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {isLoading ? 'Submitting...' : mode === 'login' ? 'Sign In' : 'Sign Up'}
              </button>
            </form>
          </div>
          ...
          <div className="px-6 py-4 bg-gray-100">
            {mode === 'login' ? (
              <p className="text-sm">
                ...
                <button onClick={switchMode} className="text-blue-600">
                  Sign Up
                </button>
              </p>
            ) : (
...
export default AuthModalIntegration;
