...
const officeAreas = [
  ...
  },
  {
    ...
    value: "GF_workspace",
    image: "/360img/IMG-20250920-WA0006.jpg",
    description: "Open workspace with a collaborative environment",
    fallbackImage: "/360img/IMG-20250920-WA0007.jpg",
  },
  {
    label: "First Floor Workspace",
...
const CompanyInterior360ViewIntegration = () => {
  const [selectedArea, setSelectedArea] = useState(officeAreas[0]);
  const [imageLoaded, setImageLoaded] = useState(false);
  const [imageError, setImageError] = useState(false);
  const [zoom, setZoom] = useState(1);
  ...
  const getImageSrc = () => (imageError ? selectedArea.fallbackImage : selectedArea.image);

  return (
    <div className="w-full py-8 px-4 bg-gray-100">
      <h2 className="text-3xl font-bold text-center mb-6">
        360° Virtual Tour - {selectedArea.label}
      </h2>
      ...
      <div className="relative bg-black rounded-lg overflow-hidden shadow-md mx-auto max-w-4xl h-96">
        {!imageLoaded && (
...
export default CompanyInterior360ViewIntegration;