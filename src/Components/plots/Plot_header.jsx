...
import { useParams } from "react-router-dom";
import { getCityPlotData, formatCityName } from "../../data/cityPropertyData";
import PlotCardSectionIntegration from "./Plot_Card_Section";
...
const PlotHeaderIntegration = () => {
  ...
  const filterOptions = {
    ...
    Locality: plotData.localities
      ? plotData.localities.slice(0, 8).map((loc) => loc.name)
      : ["Central Location", "Premium Area", "Developing Zone", "Highway Facing"],
    Builder: plotData.plots
      ? [
          ...new Set(
            plotData.plots.map((p) => p.agent || p.builder).slice(0, 6)
          ),
        ]
      : ["Premium Developers", "Trusted Builders", "Land Developers"],
    Possession: [
  ...
  return (
    <>
      <div className="bg-white py-4 px-2 shadow-sm border-t border-gray-200 pt-20">
        <div className="flex items-center gap-4 max-w-6xl mx-auto text-sm">
          <FilterButton
            ...
            label="Plot Type"
            isDropdown
            options={filterOptions["Plot Type"]}
          ...
          <FilterButton
            label="Plot Size"
            isDropdown
            options={filterOptions["Plot Size"]}
            onSelect={handleSelect}
            showDropdown={showDropdown}
            setShowDropdown={setShowDropdown}
          />
      ...
      <PlotCardSectionIntegration city={city} formattedCity={formattedCity} />
    </>
  );
};

export default PlotHeaderIntegration;