...
const FORGOT_PASSWORD_API_URL = "http://localhost:8000/api/forgot-password";
...
const ForgotPasswordIntegration = () => {
  ...
  const handleChangePassword = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${FORGOT_PASSWORD_API_URL}/change-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
  ...
  const handleResendOtp = async () => {
    setIsResending(true);
    try {
      const response = await fetch(`${FORGOT_PASSWORD_API_URL}/resend-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email }),
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-xl font-semibold text-gray-700 text-center mb-4">Forgot Password</h2>
        ...
        {!otpSent ? (
          ...
          <>
            ...
            <button
              onClick={handleResendOtp}
              disabled={isResending}
              className="w-full bg-gray-300 text-gray-700 p-2 rounded mt-2 disabled:opacity-50"
            >
